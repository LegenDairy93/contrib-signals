...
SELECT
    ...
    (
        ...
        CASE
            WHEN julianday('now') - julianday(i.updated_at) <= 30 THEN 20
            WHEN julianday('now') - julianday(i.updated_at) <= 90 THEN 10
            ELSE 0
        END +
...
WITH issue_summary AS (
    ...
    SELECT
        ...
        CAST(julianday('now') - julianday(r.pushed_at) AS INTEGER) AS days_since_push
    FROM repositories r
)
SELECT
    b.*,
    ...
    (
        CASE WHEN b.archived = 1 THEN -100 ELSE 0 END +
        CASE
        ...
        CASE WHEN b.has_code_of_conduct = 1 THEN 5 ELSE 0 END +
        CASE
